...
# 🔹 Select Best Guess
def get_best_guess(remaining_words, guessed_words):
    available_words = [word for word in remaining_words if word not in guessed_words]
...
def filter_words(last_guess, feedback, word_list):
    ...
    if len(new_list) == 0:
        matched_words = [word for word in WORDS if all(p == "_" or p == w for p, w in zip(pattern, word))]
        if matched_words:
...
async def handle_message(event):
    ...
    # 🟢 Restart Game on Win
        ...
        await client.send_message(GROUP_ID, "/new@WordSeekBot")
    ...
    # 🔎 Process Feedback
        ...
        if len(filtered_words) == 1:
            ...
            next_guess = random.choice(filtered_words)
